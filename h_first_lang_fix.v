...
Inductive Fix f : forall n:nat, Type := 
  | FixN {n} (unFixN: f (Fix f n)) : Fix f (S n)
.
Set Positivity Checking.

Definition expr := sigT (Fix exprF).


Definition contra {f a} (fixed: Fix f 0) : a :=
  match fixed with end
.
(* 
Fixpoint cata {f a n}
  (fmap: forall {b c}, (b->c) -> f b -> f c) 
  (act: f a -> a) 
  (fixed: Fix f n) {struct n} : a:=
  match n as n_ return Fix f n_ -> a with
    | S n' => fun fixed' =>
      match fixed' with
        FixN insides =>
        let cata' := @cata f a n' (@fmap) act in
          act (fmap cata' insides)
      end
    | O => contra
  end
  fixed
...
(* Fixpoint cata {f a n}
  ...
  match n as n_ return Fix f n_ -> a with
    | S n' => fun fixed' =>
      match fixed' in Fix _ _ return (_ -> a) with
        @FixN _ n__ insides => fun cata' =>
          act (fmap cata' insides)
      end (let cata' := @cata f a _ (@fmap) act in cata')
    | O => contra
  end
  fixed
. *)
...
(* Fixpoint cata {f a n}
  ...
  (act: f a -> a) 
  (fixed: Fix f n) {struct n} : a:=
  match fixed in Fix _ (S n') with
    FixN insides =>
    let cata' := @cata f a _ (@fmap) act in
      act (fmap cata' insides)
  end
. *)
...
(* Fixpoint cata {f a n}
  ...
  (act: f a -> a) 
  (fixed: Fix f n) {struct n} : a.
  move: cata fixed. destruct n. all: move=> cata. exact contra.
  case=> n' insides.
  exact (
    let cata' := @cata f a _ (@fmap) act in
      act (fmap _ _ cata' insides)
      ).
Defined. *)
